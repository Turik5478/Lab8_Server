...
void SslServer::incomingConnection(qintptr handle) {
    ...
    if (clientConnection->setSocket(handle))
    {
        ...
        connect(clientConnection, SIGNAL(sendToFile(QString,QString,QByteArray, QString)), SLOT(sendToFile(QString, QString, QByteArray, QString)));
...
void SslServer::sendToFile(QString name, QString filename, QByteArray file, QString toName)
{
    for (int i =0; i < _users.length(); i++)
    {
        if (_users[i]->_username == toName)
        {
            sendmessage(name, filename);
            QByteArray block;
            QDataStream out(&block, QIODevice::WriteOnly);
